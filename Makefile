...
og: ## Re-render og.png from tools/og.html
	$(CHROME) --headless=new --disable-gpu --hide-scrollbars \
		...
		--screenshot=/tmp/bs-og-raw.png --window-size=1200,630 tools/og.html
	$(MAGICK) /tmp/bs-og-raw.png -strip -colors 64 PNG8:og.png
	@rm -f /tmp/bs-og-raw.png
	@echo "wrote og.png"

.PHONY: favicon
favicon: ## Re-render favicon.ico and apple-touch-icon.png from the SVG sources
	$(MAGICK) -background "#$(DARK)" favicon.svg -resize 180x180 \
		-alpha off -colorspace sRGB -type TrueColor -strip apple-touch-icon.png
	$(MAGICK) \
		\( -background "#$(DARK)" favicon.svg -resize 48x48 \) \
		\( -background "#$(DARK)" favicon.svg -resize 32x32 \) \
		\( -background "#$(DARK)" tools/favicon-16.svg -resize 16x16 \) \
...
.PHONY: verify
verify: ## Check the repo for stray invisible characters (see tools/verify.py)
